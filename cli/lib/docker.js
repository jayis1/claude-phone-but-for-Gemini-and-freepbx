...
export function generateDockerCompose(config, stackId = 1) {
  ...
  // Special case for The One (Stack 4) -> 5091
  const sipPort = stackId === 4 ? 5091 : baseSipPort + (idOffset * 10);
  ...
  // Custom container name for Stack 4
...
# Stack ID: ${stackId} ${stackId === 4 ? '(The One / Fax Machine)' : ''}
# SIP Port: ${sipPort}
...
export function generateEnvFile(config, stackId = 1) {
  ...
  // SIP Port - Special case for The One
  const sipPort = stackId === 4 ? 5091 : (5060 + (idOffset * 10));
  ...
  const lines = [
    ...
    'AUDIO_DIR=/app/audio',
    `STACK_ID=${stackId}`,
    `ROLE=${stackId === 4 ? 'fax-machine' : 'voice-node'}`,
    '',
